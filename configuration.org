...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key global-map (kbd "C-`") 'toggle-shell-window)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar toggle-shell--shell-window nil)
  ...
  (defun toggle-shell--in-shell-window? ()
    "Test current buffer is shell buffer and not in full window."
    (and (toggle-shell--in-shell?)
	 toggle-shell--shell-window
	 (equal (selected-window) toggle-shell--shell-window) ; test selected window is shell window
	 ))
  ...
  (defun toggle-shell--switch-to-shell ()
    ...
    (when (toggle-shell--in-shell-window?)
      (toggle-shell--restore-window-configuration))
  ...
  (defun toggle-shell--delete-shell-window ()
    "Delete the shell buffer window"
    (when (toggle-shell--shell-buffer-existed?)
      (toggle-shell--restore-window-configuration)))

  (defun toggle-shell--switch-to-shell-window ()
    "Split the current window set the bottom one as shell."
    (unless (toggle-shell--in-shell-window?)
      (when (toggle-shell--in-shell?)
	(toggle-shell--restore-window-configuration))
      (toggle-shell--ensure-shell-buffer)
      (toggle-shell--dump-window-configuration)
      (split-window)
      (setq toggle-shell--shell-window (window-next-sibling))
  ...
  2) If in shell buffer and in half window, switch to full window.
  3) If in other buffer, switch to shell buffer in full window."
    (interactive)
    (cond ((toggle-shell--in-shell-window?) (toggle-shell--switch-to-shell))
	  ((toggle-shell--in-shell?) (toggle-shell--restore-window-configuration))
  ...
  3) If in shell buffer and in half window, delete the shell buffer window."
    (interactive)
    (cond ((toggle-shell--in-shell-window?) (toggle-shell--delete-shell-window))
	  (t (toggle-shell--switch-to-shell-window))))
#+END_SRC
