...
[[https://emacs.stackexchange.com/a/5537/29268][Is there a way to daisy chain .dir-locals.el files?]]
...
- [[https://github.com/rejeep/wrap-region.el][Github repos]]
...
More references about this

- [[https://emacs.stackexchange.com/questions/1020/problems-with-keybindings-when-using-terminal][Problems with keybindings when using terminal]]
- [[https://iterm2.com/documentation-csiu.html][CSI u | Iterm2 Docs]]
...
Supports set-mark-command

#+begin_src emacs-lisp
  (unless (display-graphic-p)
    (linw1995/global-map-and-set-key "C-@" 'set-mark-command))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun toggle-shell--ensure-shell-buffer ()
    ...
    (unless (toggle-shell--shell-buffer-existed?)
      ...
      (setq toggle-shell--shell-buffer (vterm)) ; create new shell buffer
    ...
    :hook ((python-mode
            . (lambda ()
                (setq python-shell-interpreter
                      (or (linw1995/poetry-venv-get-python-executable)
                          (linw1995/pdm-get-python-executable
                           (projectile-project-root))))
                (if-let ((extra-path (linw1995/pdm-get-packages-path
                                      (projectile-project-root))))
...
#+begin_src emacs-lisp
  ...
  (defun linw1995/pdm-get-packages-path (&optional dir)
    (let* ((pdm-get-packages-cmd "pdm info --package")
           (packages-path (concat (string-trim
                                   (shell-command-to-string
                                    (if dir
                                        (concat "cd "
                                                dir
                                      ...
                                      pdm-get-packages-cmd)))
                                  "/lib")))
      (if (file-directory-p packages-path)
          packages-path
        nil)))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package org
    ...
    (defun org-latex-export-configuration ()
    ...
    :hook
    ((org-babel-after-execute . help/org-babel-after-execute-hook))
    :config
