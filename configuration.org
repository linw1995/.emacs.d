...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package whitespace
    ...
    (custom-set-faces
     `(whitespace-tab ((t (:foreground ,(linw1995/get-color 'red))))))
         ...
         (let-alist (flycheck-count-errors flycheck-current-errors)
           ...
           (propertize (format "%s/%s/%s"
                               (propertize (int-to-string (or .error 0))
                                           'face (list :foreground (linw1995/get-color 'red)))
                               (propertize (int-to-string (or .warning 0))
                                           'face (list :foreground (linw1995/get-color 'yellow)))
                               (propertize (int-to-string (or .info 0))
                                           'face (list :foreground (linw1995/get-color 'green))))
...
#+BEGIN_SRC emacs-lisp
  (defun linw1995/poetry-get-python-executable ()
    (require 'poetry)
    (let ((venv_dir (poetry-get-virtualenv)))
      (when venv_dir (concat (file-name-as-directory venv_dir)
                             "bin/python"))))

  (defun linw1995/get-python-shell-interpreter-of-project ()
    "get python shell interpreter of project via poetry"
    (require 'projectile)
    (unless (boundp 'linw1995/project-root-2-python-shell-interpreter)
      (setq linw1995/project-root-2-python-shell-interpreter
            (make-hash-table :test 'equal)))
    (let ((project-root (projectile-project-root)))
      (unless (gethash project-root
                       linw1995/project-root-2-python-shell-interpreter)
        ;; Configure the python shell interpreter
        (puthash project-root
                 (linw1995/poetry-get-python-executable)
                 linw1995/project-root-2-python-shell-interpreter))
    ...
    :hook ((python-mode
            . (lambda ()
                (setq python-shell-interpreter
                      (linw1995/get-python-shell-interpreter-of-project))))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package python-black
    ...
    :config
    (when-let ((python-bin (linw1995/get-python-shell-interpreter-of-project))
               (black-bin (concat (file-name-directory (directory-file-name python-bin))
                                  "black")))
      (when (file-exists-p black-bin)
  ...
  (use-package python-isort
    ...
    :config
    (when-let ((python-bin (linw1995/get-python-shell-interpreter-of-project))
               (isort-bin (concat (file-name-directory (directory-file-name python-bin))
                                  "isort")))
      (when (file-exists-p isort-bin)
