...
#+BEGIN_SRC emacs-lisp
  (defun my/poetry-get-python-executable ()
    (require 'poetry)
    (let ((venv_dir (poetry-get-virtualenv)))
      (when venv_dir (concat (file-name-as-directory venv_dir)
                             "bin/python"))))

  (defun my/get-python-shell-interpreter-of-project ()
    "get python shell interpreter of project via poetry"
    (require 'projectile)
    (unless (boundp 'my/project-root-2-python-shell-interpreter)
      (setq my/project-root-2-python-shell-interpreter
            (make-hash-table :test 'equal)))
    (let ((project-root (projectile-project-root)))
      (unless (gethash project-root
                       my/project-root-2-python-shell-interpreter)
        ;; Configure the python shell interpreter
        (puthash project-root
                 (my/poetry-get-python-executable)
                 my/project-root-2-python-shell-interpreter))
    ...
    :hook ((python-mode
            . (lambda ()
                (setq python-shell-interpreter
                      (my/get-python-shell-interpreter-of-project))))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package python-black
    ...
    :config
    (when-let ((python-bin (my/get-python-shell-interpreter-of-project))
...
#+BEGIN_SRC emacs-lisp
  (use-package org-roam
    :straight t
    :init
    (defun my/org-roam-db-rebuild-cache ()
      (interactive)
      (org-roam-db-clear)
      (org-roam-db-build-cache))
    (defun my/org-roam-server ()
      (interactive)
      (server-mode)
